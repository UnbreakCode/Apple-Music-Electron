...
module.exports = {
        ...
        readFile(path, "utf-8", function (error, data) {
            if (error) {
                ...
                app.win.webContents.insertCSS(formattedData, {cssOrigin: (important ? 'user' : 'author')}).then((key) => {
                    console.log(`[${theme ? 'LoadTheme' : 'LoadCSS'}] '${fileName}' successfully injected.`)
    ...
    LoadWebsite: function (win) {
        ...
        const urlBase = app.cfg.get('advanced.useBetaSite') ? 'https://music.apple.com/de' : 'https://music.apple.com/' + app.cfg.get('general.storefront'),
